...
function createNodeStore() {
  ...
  const nextId = () =>
    `N${(nodes.reduce((m, n) => Math.max(m, parseInt(n.id.replace(/\D+/g, "") || "0", 10)), 0) + 1)
      .toString().padStart(3, "0")}`;

  const add = (x: number, y: number, status: NodeStatus = NodeStatus.Available) => {
    const n: StoredNode = { id: nextId(), x, y, status };
    nodes.push(n); save(); return n;
  };
...
function screenToDesign(
  clientX: number, clientY: number, canvas: HTMLCanvasElement, treeContainer: Container
) {
  const rect = canvas.getBoundingClientRect();
  const sx = clientX - rect.left;
  const sy = clientY - rect.top;
  const s = treeContainer.scale.x;
  return { x: (sx - treeContainer.position.x) / s, y: (sy - treeContainer.position.y) / s };
}
...
(async () => {
  ...
  Object.assign(root.style, { position: "fixed", inset: "0", overflow: "hidden", background: "#0a0a0a" } as CSSStyleDeclaration);
  ...
  Object.assign(app.canvas.style, { position: "absolute", inset: "0", width: "100%", height: "100%", zIndex: "10", cursor: "crosshair" } as CSSStyleDeclaration);
  root.appendChild(app.canvas);

  // LAYERS
  const treeContainer = new Container();
  const nodesContainer = new Container();
  const uiLayer = new Container(); uiLayer.sortableChildren = true;
  ...
  treeSprite.width = DESIGN_W; treeSprite.height = DESIGN_H;
  treeContainer.addChildAt(treeSprite, 0);

  // Water overlay (under Pixi canvas)
  const overlay = new WaterFlowOverlay({
    mount: root, pixiApp: app, treeContainer, designW: DESIGN_W, designH: DESIGN_H, dprCap: 2,
  });
  ...
  function rebuildNodes() {
    ...
    store.list().forEach((row) => {
      const n = new UINode(row.x, row.y, row.status ?? NodeStatus.Locked, row.id);
  ...
  function rebuildWires() {
    ...
    for (let i = 1; i < uiNodes.length; i++) {
      const from = uiNodes[i - 1]; const to = uiNodes[i];
      overlay.link(from.x, from.y, to.x, to.y, {
        color: 0xFFD46A, edgeColor: 0xFFF0BF, opacity: 1.0,
        radiusPx: 3.5, bend: 110, nodeRadiusPx: 26,
        endUnderlapPx: 3, taperPx: 18, glowScale: 1.18,
        speed: 0.0, streakDensity: 0.0,
      });
    }
  }
  ...
  window.addEventListener("keydown", async (ev) => {
    ...
    if (key === "k") {
      ...
      const { x: gx, y: gy } = treeContainer.toGlobal({ x: cursorDesign.x, y: cursorDesign.y });
      const meta = await openCreateNodeDialog({ app, uiLayer, attachNear: { x: gx, y: gy } });
      if (!meta) return;

      const created = store.add(cursorDesign.x, cursorDesign.y, NodeStatus.Available);

      // Save meta into per-node storage so UINode picks it up
      localStorage.setItem(`${NODE_STATE_PREFIX}${created.id}`, JSON.stringify({
        status: NodeStatus.Available,
        objectives: meta.objectives.map(t => ({ id: Math.random().toString(36).slice(2), text: t, done: false })),
        title: meta.title,
    ...
    if ((ev.ctrlKey || ev.metaKey) && key === "c") {
      if (confirm("Clear ALL saved nodes?")) {
        ...
        store.clear(); toWipe.forEach(k => localStorage.removeItem(k));
        rebuildNodes();
      }
    } else if ((ev.ctrlKey || ev.metaKey) && key === "e") {
      const rows = store.list();
      const csv = "node_id,x,y\n" + rows.map(r => `${r.id},${Math.round(r.x)},${Math.round(r.y)}`).join("\n");
